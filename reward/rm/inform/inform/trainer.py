...
class InfoRMRewardTrainer(Trainer):
	...
	def __init__(
		...
		bottleneck_loss_weight: Optional[float] = 0.1,
	):
		"""
	...
	def compute_loss(
		...
		mu_chosen = outputs_chosen["mu"]
		logvar_chosen = outputs_chosen["logvar"]
		
		rewards_rejected = outputs_rejected["logits"]
		mu_rejected = outputs_rejected["mu"]
		logvar_rejected = outputs_rejected["logvar"]
		
		# preference loss
		loss_preference = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected).mean()
		
		# bottleneck loss
		def _kl_div(mu, logvar):
			mu_mean = mu.mean(dim=-1)
			logvar_mean = logvar.mean(dim=-1)
			klds = -0.5 * (1.0 + logvar_mean - mu_mean.pow(2) - logvar_mean.exp())
			return klds.mean()

		kl_chosen = _kl_div(mu_chosen, logvar_chosen)
		kl_rejected = _kl_div(mu_rejected, logvar_rejected)
		loss_bottleneck = kl_chosen + kl_rejected

		loss = loss_preference + self.bottleneck_loss_weight * loss_bottleneck
