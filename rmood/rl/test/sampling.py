...
def has_response(item: dict) -> bool:
    return "response" in item and item["response"] not in (None, "")
...
def worker(
    ...
    try:
        ...
        for cnt, i in enumerate(pbar, start=1):
            item = data[i]
            if has_response(item):
                pbar.set_postfix_str(f"skip {i}")
                continue

            messages = deepcopy(item["messages"])

            response = llm.generate(messages, temperature=0.0, max_new_tokens=1024, top_p=1)
...
def merge_parts_into_target(source_data: list, existing_data: list, part_paths: List[str], target_path: str):
    responses = [None] * len(source_data)
    
    if existing_data:
        for i, item in enumerate(existing_data):
            if i < len(responses) and isinstance(item, dict) and "response" in item:
    ...
    atomic_write_json(responses, target_path)
