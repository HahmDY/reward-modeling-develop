...
def pad_or_trim(existing: list, target_len: int):
    ...
    if len(out) < target_len:
        out.extend(None for _ in range(target_len - len(out)))
    else:
        out = out[:target_len]
    return out
...
def worker(
    ...
    try:
        ...
        for cnt, i in enumerate(pbar, start=1):
            if existing_data and i < len(existing_data) and existing_data[i] not in (None, ""):
                pbar.set_postfix_str(f"skip {i}")
                continue

            item = deepcopy(source_data[i])
            messages = deepcopy(item["messages"])
            response = llm.generate(messages, temperature=0.0, max_new_tokens=768, top_p=1)

            partial_out[i] = response
...
def merge_parts_into_target(source_data: list, existing_data: list, part_paths: List[str], target_path: str):
    merged = existing_data or [None] * len(source_data)
...
def main_parallel(model_name: str, source_path: str, target_path: str, gpus: List[int], save_every: int = 1):
    print(f"[INFO] RL result sampling with model: {model_name}")
    ...
    print(f"[INFO] Finished sampling → {target_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_name", type=str, required=True, help="HuggingFace model name")
    parser.add_argument("--source_path", type=str, required=True, help="Path to source JSON file")
    parser.add_argument("--target_path", type=str, required=True, help="Path to save responses")
    parser.add_argument("--gpus", type=str, required=True, help="comma-separated GPU ids, e.g. '0,1,2'")
    parser.add_argument("--save_every", type=int, default=1, help="save frequency per GPU")
    ...
    main_parallel(args.model_name, args.source_path, args.target_path, gpus, args.save_every)