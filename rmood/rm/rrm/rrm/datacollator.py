...
@dataclass
class RRMDataCollatorWithPadding:
    r"""
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for feature in features:
            ...
            if (
                ...
                or "tie_label" not in feature
            ):
                raise ValueError(
                    "The features should include `input_ids_chosen`, `attention_mask_chosen`, `input_ids_rejected`, `attention_mask_rejected`, `tie_label`"
                )
            ...
            tie_label.append(feature.get("tie_label", False))
            if has_margin:
...
def add_bos_token_if_needed(
    ...
    competitive_prompt_len_input_ids: int,
    competitive_tokens: dict[str, list[int]],
):
    if bos_token_id is not None:
        ...
        if competitive_prompt_len_input_ids == 0 or bos_token_id != competitive_tokens["prompt_input_ids"][0]:
            competitive_tokens["prompt_input_ids"] = [bos_token_id] + competitive_tokens["prompt_input_ids"]
            competitive_tokens["prompt_attention_mask"] = [1] + competitive_tokens["prompt_attention_mask"]
    return prompt_tokens, chosen_tokens, rejected_tokens, competitive_tokens


def add_eos_token_if_needed(
    eos_token_id: int, chosen_tokens: dict[str, list[int]], rejected_tokens: dict[str, list[int]], competitive_tokens: dict[str, list[int]]
):
    ...
    if len(competitive_tokens["input_ids"]) == 0 or eos_token_id != competitive_tokens["input_ids"][-1]:
        competitive_tokens["input_ids"].append(eos_token_id)
        competitive_tokens["attention_mask"].append(1)
    return chosen_tokens, rejected_tokens, competitive_tokens
