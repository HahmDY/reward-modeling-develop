...
from .datacollator import RRMDataCollatorWithPadding
...
class RRMRewardTrainer(Trainer):
    ...
    def __init__(
        ...
        neutral_loss_weight: Optional[float] = 1.0,
    ):
        """
        ...
        Args:
            ...
            peft_config (`dict`, defaults to `None`):
                ...
                in a PEFT model.
            neutral_loss_weight (`float`, *optional*, defaults to `None`):
                The weight to use for the neutral loss. If None is specified, the neutral loss will not be used.
        """
        ...
        self.neutral_loss_weight = neutral_loss_weight
        ...
        if data_collator is None:
            ...
            data_collator = RRMDataCollatorWithPadding(processing_class)
    ...
    def compute_loss(
        ...
        tie_mask = torch.tensor(inputs["tie_label"]).to(rewards_chosen.device).bool()
        
        # non-tie loss: -log σ( r_c - r_r )
        if (~tie_mask).any():
            d_non_tie = rewards_chosen[~tie_mask] - rewards_rejected[~tie_mask]
            non_tie_preference_loss = -nn.functional.logsigmoid(d_non_tie).mean()
        else:
            non_tie_preference_loss = rewards_chosen.new_zeros(())

        # tie loss: -0.5*(logσ(d) + logσ(-d))
        if tie_mask.any():
            d_tie = rewards_chosen[tie_mask] - rewards_rejected[tie_mask]
            tie_preference_loss = -0.5 * (
                nn.functional.logsigmoid(d_tie) + nn.functional.logsigmoid(-d_tie)
            ).mean()
        else:
            tie_preference_loss = rewards_chosen.new_zeros(())

        loss = non_tie_preference_loss + self.neutral_loss_weight * tie_preference_loss
