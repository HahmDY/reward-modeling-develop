...
class PenaltyRMRewardTrainer(Trainer):
	...
	def compute_loss(
		...
		reward_chosen_response = outputs_chosen["logits_response"]
		reward_rejected_response = outputs_rejected["logits_response"]
		
		# preference loss
		loss_preference = -nn.functional.logsigmoid(rewards_chosen - rewards_rejected).mean()

		loss = loss_preference
